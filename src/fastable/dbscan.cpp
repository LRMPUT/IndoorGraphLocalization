 ...
 *
 */
#include "fastable/dbscan.h"

#include <iostream>

DBScan::DBScan(double _eps, uint _minPts, int _featuresFromCluster) {
	eps = _eps;
	minPts = _minPts;
...
void DBScan::expandCluster(std::vector<int> neighbourList,
	...
	for (uint j = 0; j < neighbourList.size(); j++) {
		...
		if (visited[x] != true) {
			...
			for (int k = 0; k < clusteringSetSize; k++) {
				if (!visited[k] && dist[std::max(x, k)][std::min(x, k)] < eps) {
...
int DBScan::findingClusters(int clusteringSetSize) {
	...
	for (int i = 0; i < clusteringSetSize; i++) {
		if (visited[i] != true) {
			...
			// Finding neighbours
			for (int k = 0; k < clusteringSetSize; k++) {
				if (dist[std::max(i, k)][std::min(i, k)] < eps) {
...
std::vector< std::tuple<int, int, int> > DBScan::run(std::vector<std::pair<int,int>>  clusteringSet, std::vector<std::pair<int,int>> &clusterMinMax) {
	...
	for (int i = 0; i < clusteringSetSize; i++)
		for (int j = 0; j <= i; j++)
			dist[i].push_back( std::sqrt( pow(clusteringSet[i].first - clusteringSet[j].first,2) + pow(clusteringSet[i].second - clusteringSet[j].second,2) ) );
