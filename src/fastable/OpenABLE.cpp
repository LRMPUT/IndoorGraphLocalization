...
void OpenABLE::computeForVideo(std::string videoPath[2], int startingFrameId, int numberOfFrames) {
	...
	// Places to store image descriptors
	Mat image;
	vector<Mat> trainDescriptors, testDescriptors;

	// Opening videos
	VideoCapture video[2];
	for (int i=0;i<2;i++) {
		video[i].open(videoPath[i]);
		video[i].set(CAP_PROP_POS_FRAMES, startingFrameId);
	}

	// Reading and processing train sequence
	int i = 0;
	while(i < numberOfFrames)
	{
		...
		auto start_time = chrono::high_resolution_clock::now();
			trainDescriptors.push_back(global_description(image));
		auto end_time = chrono::high_resolution_clock::now();
	...
	// Reading and processing test sequence
	i = 0;
	while (i < numberOfFrames) {
		std::cout << "Reading image : " << i << std::endl;
		video[1] >> image;
	...
	// The train sequence may contain multiple training sequence, so we need to pack the only one
	std::vector<std::vector<Mat>> vectorOfTrainDesc;
		vectorOfTrainDesc.push_back(trainDescriptors);

	// Computing Visual Place Recognition
	std::vector<Mat> openABLE_similarityMatrices, fastABLE_similarityMatrices;
	computeVisualPlaceRecognition(vectorOfTrainDesc, testDescriptors,
			openABLE_similarityMatrices, fastABLE_similarityMatrices, false);

	// Time statistics
	t_avg_description = t_description / ((float) (trainDescriptors.size()));
	t_avg_matching = t_matching
			/ ((float) (trainDescriptors.size()) * (float) (testDescriptors.size()));
	t_avg_matchingNew = t_matchingNew
				/ ((float) (trainDescriptors.size()) * (float) (testDescriptors.size()));
}
