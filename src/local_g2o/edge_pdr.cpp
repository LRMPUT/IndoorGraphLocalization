...
//        double averageAngle = normalize_theta(v1->estimate()[2] + v2->estimate()[2]) / 2.0;
		double averageAngle = normalize_theta(v1->estimate()[2] + _measurement[1] / 2.0);
...
/// Always go along the X
//        Eigen::Vector3d delta = (v1->estimate().inverse() * v2->estimate()).toVector();
//
//        _error[0] = delta[0] - estimatedStepLength * _measurement[0];
//        _error[1] = delta[1];
//        _error[2] = normalize_theta(delta[2] - _measurement[1]);
	...
	void EdgePDR::linearizeOplus() {
		...
		BaseMultiEdge<-1, VectorX>::linearizeOplus();
...
//
//		/// Local
////        Eigen::Vector3d deltaAng = (v1->estimate().inverse() * v2->estimate()).toVector();
////		double averageAngle = normalize_theta(deltaAng[2]) / 2.0;
//
//		Eigen::Vector3d delta;
//		delta[0] = v2->estimate()[0] - v1->estimate()[0];
//		delta[1] = v2->estimate()[1] - v1->estimate()[1];
